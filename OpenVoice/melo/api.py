...
MODEL_ROOT = Path(__file__).resolve().parent.parent / "checkpoints_v2" / "melotts"
SUPPORTED_LANGUAGES = {"ZH", "EN"}


class TTS(nn.Module):
    """Offline Chinese/English MeloTTS inference wrapper."""

    def __init__(self, language, device="auto", config_path=None, ckpt_path=None):
        super().__init__()
        ...
        model_dir = MODEL_ROOT / "pretrained_model" / language
        ...
        if self.language == "ZH_MIX_EN":
            import jieba
            from .text.multilingual_bert import load_model as load_bert_model

            jieba.initialize()
        else:
            from .text.english_bert import load_model as load_bert_model
        load_bert_model(self.device)
