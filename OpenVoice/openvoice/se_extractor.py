...
from faster_whisper import vad as faster_whisper_vad
import hashlib
import base64
import librosa

model_size = "medium"
auxiliary_dir = Path(__file__).resolve().parent.parent / 'checkpoints' / 'auxiliary'
...
faster_whisper_vad.get_vad_model = get_local_vad_model
...
def split_audio_vad(audio_path, audio_name, target_dir, split_seconds=10.0):
    SAMPLE_RATE = 16000
    audio_vad, _ = librosa.load(audio_path, sr=SAMPLE_RATE, mono=True)
    segments = faster_whisper_vad.get_speech_timestamps(
        audio_vad,
        vad_options=faster_whisper_vad.VadOptions(
